...
def gemini_summary(img_rgb, cnn_pred, cnn_conf, xgb_pred, fused_pred, exg, rcbi, ngrdi):
...
The AI system has produced the following results:
- CNN Prediction: {label} (confidence: {cnn_conf*100:.1f}%)
- XGBoost Prediction: {xgb_label}
- Fusion Prediction: {fused_label}
- Spectral Features:
  - ExG (Excess Green Index): {exg:.3f}
  - rCBI (Red-Blue Coral Bleaching Index): {rcbi:.3f}
  - NGRDI (Normalized Green-Red Difference Index): {ngrdi:.3f}

Based on the image and these results, write a concise 3-4 sentence summary that:
1. States clearly whether the coral appears healthy or bleached
2. Mentions what visual or spectral indicators support this
3. Notes the confidence level and whether all models agree
4. Gives a brief ecological note about what this means
...
if uploaded:
    ...
    if xgb_model:
        feats      = get_spectral_features(img_rgb)
    ...
    rcbi     = float(((R-B)/(R+B+1e-5)).mean())
    ngrdi    = float(((G-R)/(G+R+1e-5)).mean())
    ...
    with col_results:
        ...
        if xgb_model:
            ...
            for col, name, pred, prob in zip(
                ...
                [cnn_prob, xgb_prob, 0.6*cnn_prob + 0.4*xgb_prob]
            ):
            ...
            st.info("xgb_model.pkl not found — only CNN prediction available.")
    ...
    for col, name, val, tip in zip(
        ...
        [exg, rcbi, ngrdi],
        ["Higher = more green (healthy)", "Higher = more red dominant", "Higher = greener (healthy)"]
    ):
        with col:
            st.markdown(f"""
            <div class="metric-box">
